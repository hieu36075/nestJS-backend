...
export class ProfileController{
    ...
    @Post()
    async createProfile(@GetUser('id') userId: string, @Body() createProfileDTO: CreateProfileDTO): Promise<Profile>{
        return await this.profileService.createProfile(userId, createProfileDTO);
    }

    @Patch()
    async updateProfile(@GetUser('id') userId: string, @Body() updateProfileDTO : UpdateProfileDTO) : Promise<Profile>{
